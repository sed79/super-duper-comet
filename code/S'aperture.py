...
Gets median S' value under 10,000km diameter aperture over 2I from the 2D arrays of 
    ...
    for date in dates:
        ...
        for x in range(grads.shape[0]):
            for y in range(grads.shape[1]):
                if np.sqrt((x-200)**2+(y-200)**2) <= (tenthousandkm_in_pixels/2): #aperture radius of 5,000km
...
#final values save in extra/MUSES'forPlotting.csv
