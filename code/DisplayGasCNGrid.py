...
directory =  '../data/MedianCoaddedMaps/CN_ALL/' #400x400 CN maps
...
def plot():
    ...
    for ax, date in zip(axes,dates):
        hdulist = fits.open(directory+'CN_'+date+'.fits',ignore_missing_end=True)
    ...
    plt.savefig('../output_images/GasCNGrid_diff.pdf')
